...
def dev(current_user_email):
    ...
    years = np.arange(1, 11)  # Jaren 1 tot 10

    # Cost data for different product types
    multi_use_non_plastic_straws_cost = np.full(len(years), 100)  # Kosten van herbruikbare niet-plastic rietjes per jaar
    multi_use_plastic_straws_cost = np.full(len(years), 60)  # Kosten van herbruikbare plastic rietjes per jaar
    single_use_non_plastic_straws_cost = np.full(len(years), 60)  # Kosten van wegwerp niet-plastic rietjes per jaar
    single_use_plastic_straws_cost = np.full(len(years), 20)  # Kosten van wegwerp plastic rietjes per jaar

    multi_use_non_plastic_honey_cost = np.full(len(years), 300)  # Kosten van herbruikbare niet-plastic honing per jaar
    multi_use_plastic_honey_cost = np.full(len(years), 450)  # Kosten van herbruikbare plastic honing per jaar
    single_use_non_plastic_honey_cost = np.full(len(years), 2100)  # Kosten van wegwerp niet-plastic honing per jaar
    single_use_plastic_honey_cost = np.full(len(years), 1850)  # Kosten van wegwerp plastic honing per jaar
    ...
    selected_data = data[['Jaren', f'Kosten Herbruikbare Niet-Plastic {selected_category}', f'Kosten Herbruikbare Plastic {selected_category}',
